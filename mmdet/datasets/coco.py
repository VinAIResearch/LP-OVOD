...
class CocoDataset(CustomDataset):
    ...
    def __init__(
        ...
        if proposal_id_map is not None:
            self.id_idx = mmcv.load(proposal_id_map)
        super().__init__(
    ...
    def load_annotations(self, ann_file):
        ...
        self.cat_ids_base = self.coco.get_cat_ids(cat_names=self.COCO_SEEN_CLS)
        self.cat_ids_novel = self.coco.get_cat_ids(cat_names=self.COCO_UNSEEN_CLS)
        ...
        for metric in metrics:
            ...
            if metric == "proposal":
                ...
                if eval_base_novel:
                    ...
                    for split, cid_list in [
                        ("novel", self.cat_ids_novel),
                        ("base", self.cat_ids_base),
                        ("all", self.cat_ids),
                    ]:
                        cinds = []
                        for cid in cid_list:
                        ...
                        eval_results[f"AP50_split_{split}"] = mean_s
    ...
    def evaluate(
        ...
        eval_results = self.evaluate_det_segm(
            results, result_files, coco_gt, metrics, logger, classwise, proposal_nums, iou_thrs, metric_items
        )
