...
class CustomDataset(Dataset):
    ...
    def __init__(
        ...
        if self.proposals is not None and hasattr(self, "id_idx") and self.id_idx is not None:
            print("Mapping from image id to proposal id")
            if isinstance(next(iter(self.id_idx.keys())), str):
                self.proposals = [self.proposals[self.id_idx[str(img_id)]] for img_id in self.img_ids]
            else:
