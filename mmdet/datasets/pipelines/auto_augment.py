...
class Shear(object):
    ...
    def __init__(
        ...
        assert 0.0 <= max_shear_magnitude <= 1.0, (
            "Defaultly " "max_shear_magnitude should be in range [0,1]. " f"got {max_shear_magnitude}."
        )
        self.level = level
    ...
    def _shear_img(self, results, magnitude, direction="horizontal", interpolation="bilinear"):
        ...
        for key in results.get("img_fields", ["img"]):
            ...
            img_sheared = mmcv.imshear(
                img, magnitude, direction, border_value=self.img_fill_val, interpolation=interpolation
            )
    ...
    def __call__(self, results):
        ...
        self._shear_seg(
            results, magnitude, self.direction, fill_val=self.seg_ignore_label, interpolation=self.interpolation
        )
