        ...
        for metric in metrics:
            ...
            if metric == 'proposal_fast':
                ar = self.fast_eval_recall(
                    results, proposal_nums, iou_thrs, logger=None)
                log_msg = []
...
@DATASETS.register_module()
class LVISV1SplitDataset(LVISV1Dataset):
    def __init__(self, 
                 ann_file, 
                 pipeline, 
                 classes=None, 
        ...
        self.img_count_lbl = img_count_lbl
        super().__init__(ann_file, pipeline, classes, data_root, img_prefix, seg_prefix, proposal_file, test_mode, filter_empty_gt, proposal_id_map)
    ...
    def load_annotations(self, ann_file):
        ...
        for cat in self.coco.cats:
            cat_info = self.coco.cats[cat]
            if cat_info['frequency'] in self.img_count_lbl:
                cls_img_ids.extend(self.coco.cat_img_map[cat])
            else:
                self.ignore_ids.append(cat)
        self.img_ids = np.unique(cls_img_ids).tolist()
        data_infos = []
        for i in self.img_ids:
            info = self.coco.load_imgs([i])[0]
            # coco_url is used in LVISv1 instead of file_name
