...
from .class_name import coco_novel_label_ids
...
class StandardRoIHeadFinetune(StandardRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def __init__(self, train_temp=0.1, test_temp=0.07, neg_pos_ub=20, **kwargs):
    ...
    def simple_test_bboxes(self, x, img_metas, proposals, rcnn_test_cfg, rescale=False, **kwargs):
        ...
        if self.ensemble:
            cls_score = torch.where(
                self.novel_index,
                cls_score_image ** (1 - self.beta) * cls_score_text**self.beta,
