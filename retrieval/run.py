...
import faiss
from mmdet.models.roi_heads.class_name import coco_base_label_ids, coco_novel_label_ids, COCO_OVD_ALL_CLS, COCO_UNSEEN_CLS
...
def retrieve_patches_faiss(text_features):
    ...
    torch.HalfStorage.from_file("coco_clip_emb_train.pth", shared=False, size=sum(num_proposals) * clip_dim)).view(-1, clip_dim)
    ...
    all_scores = [[] for _ in range(len(text_features))]
...
def build_novel_dict_faiss_json(num_samples):
    ...
    for i, id in enumerate(range(1,len(text_embeddings)+1)):
        ...
        candidate_embeddings = candidate_embeddings / np.linalg.norm(candidate_embeddings, ord=2, axis=-1, keepdims=True)
        ...
        for k in range(len(selected_img_ids)):
           ...
           img_info = {"img_id": selected_img_ids[k], "proposal": selected_proposals[k], "features": selected_embedding[k], "cosine": sim[k]}
    ...
    for id in tqdm(saved_dict.keys()):
        ...
        for ann in saved_dict[id]:
            ...
            if img_id not in image_id_set:
                ...
                images.append({"file_name": str(img_id).zfill(12) + ".jpg", "width": width, "height": height
                               ,"id": img_id})
            width = right - left; height = bottom - top
            area = float(width * height)
            ann = {'segmentation': [], "area": area, "bbox": [float(left), float(top), float(width), float(height)],
                   'image_id': img_id, 'id': ann_id, "category_id": category_id, "iscrowd": 0, 'score': float(score), "cosine": float(cosine)}
            ann_id += 1
