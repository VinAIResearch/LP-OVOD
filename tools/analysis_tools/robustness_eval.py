...
def get_voc_style_results(filename, prints="mPC", aggregate="benchmark"):
    ...
    if "mPC" in prints:
        print("Mean Performance under Corruption [mPC] in AP50 = " f"{np.mean(mPC):0.3f}")
    if "rPC" in prints:
        print("Realtive Performance under Corruption [rPC] in % = " f"{np.mean(rPC) * 100:0.1f}")
...
def main():
    ...
    parser.add_argument(
        "--dataset", type=str, choices=["coco", "voc", "cityscapes"], default="coco", help="dataset type"
    )
    parser.add_argument(
        "--task", type=str, nargs="+", choices=["bbox", "segm"], default=["bbox"], help="task to report"
    )
    parser.add_argument(
        "--metric",
        nargs="+",
        choices=[None, "AP", "AP50", "AP75", "APs", "APm", "APl", "AR1", "AR10", "AR100", "ARs", "ARm", "ARl"],
        default=None,
