...
def voc_eval_with_return(result_file, dataset, iou_thr=0.5, logger="print", only_ap=True):
    ...
    mean_ap, eval_results = eval_map(
        det_results, annotations, scale_ranges=None, iou_thr=iou_thr, dataset=dataset_name, logger=logger
    )
...
def parse_args():
    ...
    parser.add_argument(
        "--severities", type=int, nargs="+", default=[0, 1, 2, 3, 4, 5], help="corruption severity levels"
    )
    parser.add_argument(
        "--eval",
        type=str,
    ...
    parser.add_argument(
        "--summaries", type=bool, default=False, help="Print summaries for every corruption and severity"
    )
    ...
    for corr_i, corruption in enumerate(corruptions):
        ...
        for sev_i, corruption_severity in enumerate(args.severities):
            ...
            data_loader = build_dataloader(
                dataset, samples_per_gpu=1, workers_per_gpu=args.workers, dist=distributed, shuffle=False
            )
