...
import os
import warnings
from mmdet.apis import multi_gpu_test, single_gpu_test_analysis
...
def parse_args():
    ...
    parser.add_argument("--scale", type=float, default=1.0, help="output result file in pickle format")
    parser.add_argument(
        "--fuse-conv-bn",
        action="store_true",
...
def main():
    ...
    if not distributed:
        ...
        outputs = single_gpu_test_analysis(
            model, data_loader, args.show, args.show_dir, args.show_score_thr, args.scale
        )
    else:
