...
def bbox_map_eval(det_result, annotation, nproc=4):
    ...
    for thr in iou_thrs:
        p = workers.apply_async(
            eval_map, (bbox_det_result, [annotation]), {"iou_thr": thr, "logger": "silent", "nproc": 1}
        )
        processes.append(p)
...
class ResultVisualizer:
    ...
    def evaluate_and_show(self, dataset, results, topk=20, show_dir="work_dir"):
        ...
        if isinstance(results[0], dict):
            ...
            raise "The format of result is not supported yet. " "Current dict for panoptic segmentation and list " "for object detection are supported."
...
def parse_args():
    ...
    parser.add_argument(
        "--topk", default=20, type=int, help="saved Number of the highest topk " "and lowest topk after index sorting"
    )
