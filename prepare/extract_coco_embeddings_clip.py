...
class CocoCroppedProposals(CocoDataset):
    ...
    def get_bboxes_and_bboxes15(self, bboxes, h, w):
        def box_coord_2int(box):
            box = (np.floor(box[0]-0.001),np.floor(box[1]-0.001),np.ceil(box[2]+0.001),np.ceil(box[3]+0.001))
            return (
                max(box[0], 0),
                max(box[1], 0),
                min(box[2], w),
...
def main():
    ...
    embeddings = torch.HalfTensor(torch.HalfStorage.from_file(args.save_path, shared=True, size=len(dataset) * clip_model.visual.output_dim)).reshape(len(dataset), -1)
    start = time.time()
    with torch.no_grad():
